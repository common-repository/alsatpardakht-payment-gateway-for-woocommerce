...
function Load_AlsatPardakht_Gateway()
{
    ...
    if (!function_exists('Woocommerce_Add_AlsatPardakht_Gateway') && class_exists('WC_Payment_Gateway') && !class_exists('WC_ALSATPARDAKHT')) {
        ...
        add_filter('woocommerce_currencies', 'alsatpardakht_add_IR_currency');

        function alsatpardakht_add_IR_currency($currencies)
        {
        ...
        add_filter('woocommerce_currency_symbol', 'alsatpardakht_add_IR_currency_symbol', 10, 2);

        function alsatpardakht_add_IR_currency_symbol($currency_symbol, $currency)
        {
            switch ($currency) {
                case 'IRR':
        ...
        class WC_ALSATPARDAKHT extends WC_Payment_Gateway
        {
            ...
            public function Send_to_AlsatPardakht_Gateway($order_id)
            {
                ...
                $Fault = '';
                ?>
                <form action="" method="POST" class="AlsatPardakht-checkout-form" id="AlsatPardakht-checkout-form"
                      style="direction:ltr;">
                    <input type="submit" name="AlsatPardakht_submit" class="button alt"
                           id="AlsatPardakht-payment-button" value="<?php echo __('پرداخت', 'woocommerce') ?>"/>
                    <a class="button cancel" href="<?php wc_get_checkout_url() ?>"><?php echo __('بازگشت',
                            'woocommerce') ?></a>
                </form><br/>
                <?php
